...
type LoadBalance struct {
	...
	CheckTimeout           int    `json:"check_timeout" gorm:"column:check_timeout" description:"check 超时时间"`
	CheckInterval          int    `json:"check_interval" gorm:"column:check_interval" description:"检查间隔 单位s"`
	RoundType              int    `json:"round_type" gorm:"column:round_type" description:"轮询方式 round/weightRound/random/ipHash"`
...
func (lb *LoadBalance)Save(c *gin.Context, db *gorm.DB) error {
	if err := db.SetCtx(public.FromGinTraceContext(c)).Save(lb).Error;err!=nil{
		return err
	}
	return nil
}
...
func (lb *LoadBalance)GetWeightListByModel() []string {
